...
function generateRandomVectors(count, dims) {
  ...
  for (let i = 0; i < vectors.length; i++) {
    vectors[i] = Math.random() * 2 - 1; // Random values in [-1, 1]
  }
  return vectors;
}

function generateRandomQuery(dims) {
...
async function benchmarkAdd(indexType, vectorCount, dims) {
  ...
  if (indexType === 'IVF_FLAT') {
    ...
    const trainingVectors = generateRandomVectors(Math.min(vectorCount, 1000), dims);
  ...
  const vectors = generateRandomVectors(vectorCount, dims);
  
  // Warmup (first operation can be slower)
  if (indexType !== 'IVF_FLAT') {
    const warmupVectors = generateRandomVectors(Math.min(100, vectorCount), dims);
...
async function benchmarkSearch(index, indexType, queryCount, k) {
  ...
  const queries = generateRandomVectors(queryCount, dims);
  
  // Warmup
  const warmupQuery = generateRandomQuery(dims);
  await index.search(warmupQuery, k);
  
  // Single query benchmark (if queryCount === 1)
  if (queryCount === 1) {
    const query = generateRandomQuery(dims);
    const start = process.hrtime.bigint();
...
async function benchmarkJSOverhead() {
  ...
  const vectors = generateRandomVectors(100, dims);
  await index.add(vectors);
...
async function runBenchmarks() {
  ...
  console.log('='.repeat(70));
  console.log('\nNote: Actual performance depends on hardware, FAISS build configuration,');
  console.log('and system load. These benchmarks provide a baseline for comparison.');
}
